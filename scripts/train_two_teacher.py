...
# local import
from scripts.helpers import make_env_policy, EpisodeStats, evaluate, parse_checkpoint_path
...
@hydra.main(config_path=CONFIG_PATH, config_name="train_two_teacher", version_base=None)
def main(cfg: DictConfig):
    ...
    checkpoint_legs = torch.load(parse_checkpoint_path(cfg.checkpoint_path_legs))
    checkpoint_arm = torch.load(parse_checkpoint_path(cfg.checkpoint_path_arm))

    vecnorm_legs = VecNorm(vecnorm.in_keys, decay=0.9999)
    vecnorm_legs(env.fake_tensordict())
    vecnorm_legs.load_state_dict(checkpoint_legs["vecnorm"])

    vecnorm_arm = VecNorm(vecnorm.in_keys, decay=0.9999)
    vecnorm_arm(env.fake_tensordict())
    vecnorm_arm.load_state_dict(checkpoint_arm["vecnorm"])

    policy_legs = deepcopy(policy)
    policy_legs.load_state_dict(checkpoint_legs["policy"])

    # teacher_legs = policy_legs.get_rollout_policy("eval")
    ...
    arm_joint_ids, arm_joint_names = env.robot.find_joints("arm_joint[1-5]")
    legs_joint_ids, legs_joint_names = env.robot.find_joints("(F|R)(L|R)_(hip|thigh|calf)_joint")

    policy_arm = deepcopy(policy)
    policy_arm.load_state_dict(checkpoint_arm["policy"])

    from active_adaptation.learning.ppo.ppo_dic import PPODICPolicy

    policy: PPODICPolicy = policy
    policy.set_two_teacher(
        policy_legs.get_rollout_policy("eval"),
        policy_arm.get_rollout_policy("eval"),
        legs_joint_ids,
        arm_joint_ids,
        cfg.reg_lambda_legs,
        cfg.reg_lambda_arm,

...
class JointPositionAndPhase(ActionManager):
    ...
    def reset(self, env_ids: torch.Tensor):
        ...
        self.asset.phi[env_ids] = torch.rand(len(env_ids), 4, device=self.device) * torch.pi * 2.0
    ...
    def __call__(self, tensordict: TensorDictBase, substep: int):
        if substep == 0:
            ...
            self.action_buf[:, :, 1:] = self.action_buf[:, :, :-1]
            self.action_buf[:, :, 0] = action
            action = self.action_buf.take_along_dim(self.delay.unsqueeze(1), dim=-1).squeeze(-1)
            
            self.applied_action.lerp_(action, self.alpha)

            pos_target = self.default_joint_pos.clone()
            jpos_des, phase = self.applied_action.split([12, 1], dim=-1)
            pos_target[:, self.joint_ids] += jpos_des * self.action_scaling
            if hasattr(self, "cumstom_command"):
                pos_target[:, self.cumstom_command_joint_ids] = self.cumstom_command
            if hasattr(self, "clip_joint_targets"):
                pos_target = self.asset.data.joint_pos + (pos_target - self.asset.data.joint_pos).clamp(-self.clip_joint_targets, self.clip_joint_targets)
            ...
            self.asset.phi_dot[:] = self.omega * phase.sigmoid() # + torch.tanh(phase * 0.25) * 2 * torch.pi
            self.asset.phi[:] = (self.asset.phi + self.asset.phi_dot * self.env.step_dt) % (2 * torch.pi)

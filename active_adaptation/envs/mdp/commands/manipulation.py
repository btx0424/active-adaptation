...
class EEImpedance(Command):
    ...
    def __init__(
        ...
        self.fix_reset_body_pos = fix_reset_body_pos
    ...
    def _debug_draw_setpoint_boundaries(self):
        ...
        setpoint_bounds_world = (
            setpoint_bounds.unsqueeze(0) + self.asset.data.root_pos_w.unsqueeze(1)
        ).view(-1, 3)
...
class PushWall(Command):
    ...
    def save_logs(self):
        ...
        with open(self.log_file, "wb") as f:
            ...
            pickle.dump(
                (actual_forces, desired_forces, desired_kp_forces, kp_forces), f
            )
    ...
    def _sample_wall(self, env_ids: torch.Tensor):
        ...
        wall_normal_yaw = torch.empty(len(env_ids), device=self.device).uniform_(
            -torch.pi / 6, torch.pi / 6
        )
        wall_normal_pitch = torch.empty(len(env_ids), device=self.device).uniform_(
            -torch.pi / 6, 0
        )
        ...
        wall_distance = torch.empty(len(env_ids), 1, device=self.device).uniform_(
            0.3, 0.6
        )
    ...
    def step(self, substep: int):
        ...
        wall_resistance = self.wall_kp * (ee_distance - self.wall_distance).clamp_min(
            0.0
        )
        ...
        velocity_parallel = (
            body_vel
            - (body_vel * self.wall_direction).sum(dim=-1, keepdim=True)
            * self.wall_direction
        )
        velocity_parallel[velocity_parallel.norm(dim=-1) < 1e-3] = 0.0
        velocity_parallel_dir = velocity_parallel / velocity_parallel.norm(
            dim=-1, keepdim=True
        ).clamp_min(1e-3)
    ...
    def _debug_draw_wall(self):
        ...
        wall_points_b = quat_rotate(
            wall_quat.view(-1, 4), wall_points_b.view(-1, 3)
        ).view(-1, num_points, 3)

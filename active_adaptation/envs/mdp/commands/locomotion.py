...
class Command1(Command):
    ...
    def update(self):
        ...
        yaw_diff = self.target_yaw - self.ref_yaw
        self.command_angvel[:] = self.yaw_stiffness * math_utils.wrap_to_pi(yaw_diff)
        self.command_angvel.clamp_(-2., 2.)

        max_speed = (self.max_speed - self.command_angvel.abs()).clamp(0.)
        pos_diff = self.target_pos_w - pos
        pos_diff[:, 2] = 0.0
        self.command_linvel_w[:, :2] = clamp_norm(pos_diff[:, :2], max=max_speed)
        self.command_linvel_b[:] = quat_rotate_inverse(self.asset.data.root_quat_w, self.command_linvel_w)
        self.command[:, :2] = self.command_linvel_b[:, :2]
        self.command[:, 2:3] = math_utils.wrap_to_pi(self.ref_yaw - yaw)
...
class Command2(Command):
    ...
    def update(self):
        ...
        if self.teleop:
            ...
            interval_reached = (
                self.env.episode_length_buf - 40
        ...
        self.command_linvel_w[:] = quat_apply_yaw(
            self.robot.data.root_quat_w, self.command_linvel
        )
...
class InterpCommand(Command):
    ...
    def __init__(
        ...
        with torch.device(self.device):
            ...
            self.ray_directions = torch.tensor([0.0, 0.0, -1.0])
            ...
            self.last_init_root_state = torch.zeros(
    ...
    def reset(self, env_ids):
        self.command[env_ids] = 0.0
        self.command_speed[env_ids] = 0.0
        self.command_linvel[env_ids] = 0.0
        ...
        self.interp_pos[env_ids] = self.pos_w_history[env_ids].lerp(
            self.integrated_pos_w_history[env_ids], self.alpha
        )
        yaw_diff = self.integrated_yaw_history[env_ids] - self.yaw_history[env_ids]
        self.interp_yaw[env_ids] = self.yaw_history[
            env_ids
        ] + self.alpha * math_utils.wrap_to_pi(yaw_diff)
    ...
    def update(self):
        ...
        self.command_linvel.add_(
            clamp_norm((self.target_linvel - self.command_linvel) * 0.1, max=0.1)
        )
        self.command_speed[:] = self.command_linvel.norm(dim=-1, keepdim=True)
        ...
        command_linvel_w = quat_rotate(
            quat_from_yaw(self.integrated_yaw), self.command_linvel
        )
        ...
        self._cum_error[:, 0] = (
            self.integrated_pos_w - self.asset.data.root_pos_w
        ).norm(dim=-1)
    ...
    def sample_vel_command(self, env_ids: torch.Tensor):
        ...
        self.target_linvel[env_ids, :2] = torch.where(speed < 0.2, linvel * 0.0, linvel)
    ...
    def _height(self, pos_w: torch.Tensor):
        ...
        ray_hits_w = raycast_mesh(
            ray_starts, ray_directions, max_dist=100, mesh=self.ground_mesh
        )[0]
    ...
    def debug_draw(self):
        ...
        for i in range(self.num_envs):
            if self.use_interp[i]:
                ...
                self.env.debug_draw.plot(target_pos.T, color=(0.0, 1.0, 1.0, 1.0))
                interp_yaw = self.interp_yaw[i, : t[i].item()]
                vec = torch.stack(
                    [
                        torch.cos(interp_yaw),
                        torch.sin(interp_yaw),
                        torch.zeros_like(interp_yaw),
                    ],
                    1,
                )
                self.env.debug_draw.vector(
                    target_pos.T, vec, color=(0.0, 1.0, 1.0, 0.5)
                )
            else:
                ...
                vec = torch.stack(
                    [
                        torch.cos(integrated_yaw),
                        torch.sin(integrated_yaw),
                        torch.zeros_like(integrated_yaw),
                    ],
                    1,
                )
                self.env.debug_draw.vector(
            ...
            self.env.debug_draw.plot(pos.T, color=(1.0, 1.0, 0.0, 1.0))
        ...
        self.env.debug_draw.vector(
            root_pos_w + torch.tensor([0.0, 0.0, 0.2], device=self.device),
...
class Impedance(Command):
    ...
    def __init__(
        ...
        with torch.device(self.device):
            ...
            self.impulse_force_momentum_scale = (
                torch.tensor(impulse_force_momentum_scale) * self.default_mass
            )
            ...
            self.constant_force_struct = torch.zeros(
                self.num_envs, 3 + 1 + 1
            )  # force, duration, time
            self.constant_force_duration = self.constant_force_struct[:, -2].unsqueeze(
                1
            )
            self.constant_force_time = self.constant_force_struct[:, -1].unsqueeze(1)

            self.impulse_force_struct = torch.zeros(
                self.num_envs, 3 + 1 + 1
            )  # force, duration, time
    ...
    def reset(self, env_ids: torch.Tensor):
        ...
        self.desired_yaw_w[env_ids] = self.asset.data.heading_w[env_ids].reshape(
            -1, 1, 1
        )
    ...
    def _integrate(self, dt: float):
        desired_acc_w = (
            ...
            * (self.command_setpos_w.unsqueeze(1) - self.desired_pos_w)
            + self.kd.unsqueeze(1) * (0.0 - self.desired_lin_vel_w)
        ...
        desired_yaw_acc_w = (
            ...
            + self.kd.unsqueeze(1) * (0.0 - self.desired_yaw_vel_w)
    ...
    def update(self):
        ...
        constant_force = self.constant_force_struct[:, 0:3] * (
            self.constant_force_time < self.constant_force_duration
        )
        impulse_force_t = (self.impulse_force_time / self.impulse_force_duration).clamp(
            0.0, 1.0
        )
        ...
        linvel_noise = (
            torch.randn_like(self.asset.data.root_lin_vel_b).clip(-1, 1) * 0.1
        )
        yaw_diff = math_utils.wrap_to_pi(
            self.command_setrpy_w[:, 2] - self.asset.data.heading_w
        )
        command_yaw_diff = math_utils.wrap_to_pi(
            self.command_yaw_w - self.asset.data.heading_w.unsqueeze(1)
        )
        ...
        self.command[:, 5:8] = (
            self.kd
        )  # * - (self.asset.data.root_lin_vel_b + linvel_noise)
        ...
        if self.teleop:
            ...
            self.command_setpos_w[:] = torch.where(
                ...
                self.command_setpos_w.lerp(
                    self.kd / self.kp * self.set_linvel + self.asset.data.root_pos_w,
                    0.5,
                ),
        ...
        force_type = torch.multinomial(
            self.force_type_probs, self.num_envs, replacement=True
        )
    ...
    def _sample_command(self, env_ids: torch.Tensor):
        ...
        kp = torch.empty(len(env_ids), 1, device=self.device).uniform_(
        ...
        command_setpoint_yaw = torch.zeros(len(env_ids), device=self.device).uniform_(
            -torch.pi / 2, torch.pi / 2
        )
        self.command_setrpy_w[env_ids, 2] = math_utils.wrap_to_pi(
            command_setpoint_yaw + self.asset.data.heading_w[env_ids]
        )
...
def maybe_sample_force(
    ...
    if sample_force[tid]:
        if force_type[tid] == 0:
            ...
            const_force[tid] = vec5f(xy[0], xy[1], 0.0, duration, 0.0)
            ...
            xy = xy / duration
            impulse_force[tid] = vec5f(xy[0], xy[1], 0.0, duration, 0.0)

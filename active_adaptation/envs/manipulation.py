...
class QuadrupedManip(LocomotionEnv):
    ...
    def __init__(self, cfg):
        ...
        self.arm = self.robot.actuators["arm"]
        self.actuated_joint_indices = (
            self.base_legs.joint_indices + self.arm.joint_indices
        )
        logging.info(f"Actuated joint indices: {self.actuated_joint_indices}")

        self.default_joint_pos = self.robot.data.default_joint_pos[0, self.actuated_joint_indices]
    ...
    def apply_action(self, tensordict: TensorDictBase, substep: int):
        if substep == 0:
            ...
            self.robot.set_joint_position_target(pos_target, self.actuated_joint_indices)
    ...
    def joint_pos(self):
        ...
        return random_noise(all_joint_pos, 0.05)
    
    @observation_func
    def joint_vel(self):
        all_joint_vel = self.robot.data.joint_vel
        return random_noise(all_joint_vel, 0.2)
